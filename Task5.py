...
class Iterator_Task1:
    """It's constructor"""
    def __init__(self, path: str) -> None: #self ссылка на сам только что созданный объект
        self.file_names = os.listdir(os.path.join('dataset', path)) #Функция возвращает список, содержащий имена файлов и директорий в каталоге, заданном путем
        self.counter = 0
...
class IteratorTask3:
    ...
    def __init__(self, class_name: str, path: str, annotation_name: str) -> None:
        ...
        with open(os.path.join(path, annotation_name), encoding='UTF-16') as file:
            reader = csv.reader(file, delimiter=',') #Возвращает объект чтения, который будет перебирать объект чтения в файле
